...
function resolveTree (symbols /*: BabelSymbol[] */) /*: BabelSymbolTree */ {
  ...
  for (let i = 0; i < parentResolveQueue.length; i++) {
    ...
    // References to variables are removed
    if (resolveMaybeVariableReference(symbol.object, symbol)) {
      continue
    }
...
// Before resolveObject, this checks if an assignment is done to a inner scope symbol. These
...
//   const scope = {};
//
//   // Symbol "prop" is ignored
//   scope.prop = 'val';
// }
function resolveMaybeVariableReference (longname, refereeSymbol) {
  const id = longname.split('.')[0]
  ...
  while (parent) {
    if (parent.nestLookup[id]) {
      const referredSymbol = parent.nestLookup[id]

      if (referredSymbol.scope === 'inner') {
        return true
      }
    }
