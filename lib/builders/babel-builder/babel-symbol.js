...
export type BabelSymbol = {
  ...
  object?: 'this' | string,
  type: 'class' | 'function' | 'method' | 'object' | 'string' | 'number' | 'boolean' | 'any' | 'param',
}
*/
...
export function fromBabel (nodePath /*: NodePath */, options /*: any */) /*: ?BabelSymbol */ {
  ...
  if (isDeclaration(node) || isVariableDeclarator(node)) {
    ...
    scope = 'static'
    type = 'method'
  } else if (node.name && nodePath.parent && isBabelFunction(nodePath.parent) &&
nodePath.parent.params.indexOf(node) >= 0) {
    name = node.name
    scope = 'inner'
    type = 'param'
  } else {
